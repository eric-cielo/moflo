...
// Also prunes top-level mirror files whose source no longer exists in shipped/
// (e.g. after a guidance rename: moflo-workflow-* → moflo-spell-*) so stale
// mirrors don't linger across upgrades when neither version-bump nor manifest
// drift fires (#839). Pre-manifest-tracking mirrors never appeared in
// installed-files.json, so the section-3 manifest-diff cleanup can't reach
// them; this unconditional prune self-heals every session.
try {
  const guidanceDir = resolve(projectRoot, '.claude/guidance');
  const shippedDir = resolve(projectRoot, 'node_modules/moflo/.claude/guidance/shipped');
  if (existsSync(shippedDir)) {
    ...
    // Prune stale mirrors. A top-level `.claude/guidance/*.md` is launcher-
    // managed iff its first line is the session-start AUTO-GENERATED marker.
    // If its filename no longer exists in shipped/, the source was renamed
    // or removed and the mirror is stale. Files without the marker — user-
    // authored notes, plus `moflo-bootstrap.md` which uses the distinct
    // `AUTO-GENERATED by moflo init` marker — are never touched.
    if (existsSync(guidanceDir)) {
      ...
      for (const file of readdirSync(guidanceDir)) {
        ...
        try {
          if (!readFileSync(abs, 'utf-8').startsWith(SESSION_START_MARKER)) continue;
          unlinkSync(abs);
          prunedStale++;
