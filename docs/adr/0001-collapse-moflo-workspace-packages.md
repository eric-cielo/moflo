...
- **Leaves (zero outbound `@moflo/*` imports):** `aidefence`, `claims`, `embeddings`, `guidance`, `plugins`, `security`, `shared`, `spells`, `swarm` (9/14 — `aidefence` inlined in #590, `claims` deleted in #591, `embeddings` inlined in #592, `plugins` deleted in #593, `guidance` inlined in #600)
- **Mid-tier:** `neural` → `memory`; `hooks` → `memory`, `security`; `testing` → `memory`, `shared`, `swarm`
