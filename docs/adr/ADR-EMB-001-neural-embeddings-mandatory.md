...
1. **ESLint identifier rule** (`eslint.config.js`) — flags any occurrence of the banned identifiers above (`HashEmbeddingProvider`, `createHashEmbedding`, `generateHashEmbedding`, `RvfEmbedding*`, `domain-aware-hash-*`) in `src/` and `bin/`.
...
3. **ESLint structural rule** (`eslint.config.js`, added in PR #543, unscoped across `src/**/*.ts` in #545) — `no-restricted-syntax` with `:has()` selectors flags any function that both constructs a `Float32Array` and calls `charCodeAt(...)`. Catches the anti-pattern regardless of method name.
...
5. **DI-compliance test** (`src/cli/__tests__/embeddings/provider-di-compliance.test.ts`, added in #546) — pins the positive contract: every known semantic-embedding entry point (`ShardRetriever`, hierarchical-memory controller, attention/queen coordinators, hooks ReasoningBank) must reject a missing `IEmbeddingProvider` at construction or first `embed()` call. Catches the anti-pattern when someone reintroduces an ambient / default-constructed provider.
...
- **2026-04-23 — #545.** Architect review unscoped the `Float32Array + charCodeAt` structural rule across the entire `src/` tree (not just swarm) and added the newly found method names to the banned-identifier list. This surfaced additional live paths in `src/cli/mcp-tools/hooks-tools.ts` (`generateSimpleEmbedding` + inline MoE / Flash-Attention demo branches) and `src/cli/embeddings/embedding-service.ts` (`MockEmbeddingService.deterministicEmbedding` reachable via factory `case 'mock'`).
- **2026-04-23 — #546.** Full audit of every `Float32Array` producer under `src/` (PR #546; the standalone audit file was retired in #661 once `provider-di-compliance.test.ts` became the live source of truth). Each producer was classified as **PROVIDER** (flows through `IEmbeddingProvider`), **NON-SEMANTIC** (RL state, SONA LoRA, MoE weights, HNSW index), or **NEEDS-FIX**. Adds `src/cli/__tests__/embeddings/provider-di-compliance.test.ts` — a DI-compliance test that pins the contract: every known semantic-embedding entry point must reject a missing provider at construction or first `embed()` call.
- **2026-04-24 — #558 / #560.** Purge of the remaining NEEDS-FIX sites surfaced by the strengthened guard. #558 covered the known `hooks-tools.ts` + migration fixture sites; #560 tracks the newly discovered `src/cli/benchmarks/pretrain/index.ts` benchmark that reported hash-based "embeddings" as real `ops/sec` in a user-visible CLI metric.
...
- `src/cli/embeddings/fastembed-embedding-service.ts` — production service
...
- `src/cli/__tests__/embeddings/provider-di-compliance.test.ts` — DI-compliance test pinning the provider-required contract (live source of truth — replaced the standalone audit doc in #661)
