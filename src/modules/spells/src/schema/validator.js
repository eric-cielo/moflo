...
const VALID_ARG_TYPES = ['string', 'number', 'boolean', 'string[]'];
/**
...
// ============================================================================
function validateTopLevel(def, errors) {
...
// Argument validation
// ============================================================================
function validateArguments(args, errors) {
...
// Step validation
// ============================================================================
function validateSteps(steps, errors, stepIds, outputVars, options, prefix = 'steps', workflowLevel) {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (step.steps && Array.isArray(step.steps)) {
            ...
            for (const nested of step.steps) {
                if (nested.id)
                    declaredStepIds.push(nested.id);
            }
        }
    }
}
function checkStringReferences(value, declaredStepIds, args, errors, path) {
    if (!errors || !path)
        return;
 ...
 */
function validateParallelCrossReferences(steps, errors, prefix) {
 ...
 */
export function resolveArguments(definitions, provided) {
    const resolved = {};
    const errors = [];
...
function matchesArgumentType(value, type) {
    switch (type) {
 ...
 */
function detectCircularJumps(steps, errors) {
