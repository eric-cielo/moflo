...
export function interpolateString(template, context) {
    return template.replace(VAR_REF_PATTERN, (match, path) => {
 ...
 * @throws if a referenced variable is not found.
 */
export function shellInterpolateString(template, context) {
    return template.replace(VAR_REF_PATTERN, (match, path) => {
        const value = resolveVariable(path, context);
        if (value === undefined) {
            throw new Error(`Variable not found: ${path}`);
 ...
 */
export function interpolateConfig(config, context) {
    const result = {};
