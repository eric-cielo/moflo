...
export const VALID_CAPABILITY_TYPES = new Set([
 ...
 */
export function checkCapabilities(step, command) {
    ...
    for (const [capType, scope] of Object.entries(stepRestrictions)) {
        ...
        if (!commandCapTypes.has(capType)) {
            violations.push({
                capability: capType,
 ...
 */
export function enforceScope(effectiveCaps, capabilityType, resource, stepId, stepType) {
...
const VALID_MOFLO_LEVELS = new Set(MOFLO_LEVEL_ORDER);
/**
 * Check if a string is a valid MofloLevel.
 */
export function isValidMofloLevel(level) {
 ...
 */
export function compareMofloLevels(a, b) {
 ...
 */
export function getDefaultMofloLevel(_stepType, command) {
 ...
 */
export function resolveMofloLevel(step, command, workflowLevel, parentLevel) {
...
//# sourceMappingURL=capability-validator.js.map