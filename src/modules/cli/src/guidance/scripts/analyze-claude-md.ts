...
import { analyze, benchmark, autoOptimize, formatReport, formatBenchmark } from '../analyzer.js';

function findRepoRoot(start: string): string {
  let dir = start;
  while (dir !== dirname(dir)) {
    if (existsSync(resolve(dir, 'package.json')) && existsSync(resolve(dir, 'src', 'modules'))) return dir;
    dir = dirname(dir);
  }
...
const V3_CLAUDE_MD = resolve(REPO_ROOT, 'src', 'CLAUDE.md');
