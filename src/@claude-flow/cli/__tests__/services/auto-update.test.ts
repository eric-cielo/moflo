...
describe('auto-update', () => {
  ...
  describe('shipped guidance syncing', () => {
    it('should sync all .md files from shipped/ to .claude/guidance/ with moflo- prefix', () => {
      scaffoldProject({
        ...
        guidanceFiles: {
          ...
          'task-swarm-integration.md': '# Swarm',
        },
      });
      ...
      expect(existsSync(join(tempDir, '.claude/guidance/moflo-agent-bootstrap.md'))).toBe(true);
      expect(existsSync(join(tempDir, '.claude/guidance/moflo-memorydb-maintenance.md'))).toBe(true);
      expect(existsSync(join(tempDir, '.claude/guidance/moflo-task-swarm-integration.md'))).toBe(true);
    });

    it('should prepend auto-generated header to synced guidance files', () => {
      scaffoldProject({
        mofloVersion: '4.8.0',
        guidanceFiles: { 'agent-bootstrap.md': '# Bootstrap Content' },
      });

      simulateAutoUpdate(tempDir);

      const content = readFileSync(join(tempDir, '.claude/guidance/moflo-agent-bootstrap.md'), 'utf-8');
      expect(content).toContain('AUTO-GENERATED by moflo session-start');
      expect(content).toContain('Do not edit');
      expect(content).toContain('node_modules/moflo/.claude/guidance/shipped/agent-bootstrap.md');
    ...
    it('should overwrite guidance files on version change', () => {
      scaffoldProject({
        mofloVersion: '4.8.0',
        guidanceFiles: { 'agent-bootstrap.md': '# Version 1' },
      });

      simulateAutoUpdate(tempDir);
      const v1 = readFileSync(join(tempDir, '.claude/guidance/moflo-agent-bootstrap.md'), 'utf-8');
      ...
      writeFileSync(
        join(tempDir, 'node_modules/moflo/.claude/guidance/shipped/agent-bootstrap.md'),
        '# Version 2',
      );

      const result = simulateAutoUpdate(tempDir);
      expect(result.synced).toBe(true);
      const v2 = readFileSync(join(tempDir, '.claude/guidance/moflo-agent-bootstrap.md'), 'utf-8');
