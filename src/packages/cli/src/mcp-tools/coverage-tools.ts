 ...
 * Integrates with hooks_coverage_route and hooks_coverage_suggest from movector.
 */
...
} from '../movector/coverage-router.js';

/**
 * Coverage-aware routing MCP tool
 *
 * Routes tasks to optimal agents based on test coverage gaps.
 * Uses movector's hooks_coverage_route when available.
 */
export const hooksCoverageRoute: MCPTool = {
  name: 'hooks/coverage-route',
  description: 'Route task to agents based on test coverage gaps (movector integration)',
  category: 'coverage',
  tags: ['coverage', 'routing', 'testing', 'movector'],
  inputSchema: {
    type: 'object',
    properties: {
      task: {
      ...
      useNativeBackend: {
        type: 'boolean',
        description: 'Use movector integration if available (default: true)',
      },
    },
    required: ['task'],
  },
  handler: async (params: Record<string, unknown>): Promise<CoverageRouteResult> => {
    ...
    const useNativeBackend = params.useNativeBackend as boolean | undefined;
 ...
 * Uses movector's hooks_coverage_suggest when available.
 */
export const hooksCoverageSuggest: MCPTool = {
  name: 'hooks/coverage-suggest',
  description: 'Suggest coverage improvements for a path (movector integration)',
  category: 'coverage',
  tags: ['coverage', 'suggestions', 'testing', 'movector'],
  inputSchema: {
    type: 'object',
    properties: {
      path: {
