...
function simulateAutoUpdate(projectRoot: string) {
  ...
  if (existsSync(shippedDir)) {
    ...
    for (const file of shippedFiles) {
      ...
      const dest = join(guidanceDir, file);
...
describe('auto-update', () => {
  ...
  describe('shipped guidance syncing', () => {
    it('should sync all .md files from shipped/ to .claude/guidance/ preserving filenames', () => {
      scaffoldProject({
        mofloVersion: '4.8.0',
        guidanceFiles: {
          'moflo-subagents.md': '# Bootstrap',
          'moflo-memorydb-maintenance.md': '# MemoryDB',
      ...
      expect(existsSync(join(tempDir, '.claude/guidance/moflo-claude-swarm-cohesion.md'))).toBe(true);
    });

    it('should prepend auto-generated header to synced guidance files', () => {
      scaffoldProject({
        mofloVersion: '4.8.0',
        guidanceFiles: { 'moflo-subagents.md': '# Bootstrap Content' },
      });
      ...
      expect(content).toContain('node_modules/moflo/.claude/guidance/shipped/moflo-subagents.md');
    ...
    it('should overwrite guidance files on version change', () => {
      scaffoldProject({
        mofloVersion: '4.8.0',
        guidanceFiles: { 'moflo-subagents.md': '# Version 1' },
      });
      ...
      writeFileSync(
        join(tempDir, 'node_modules/moflo/.claude/guidance/shipped/moflo-subagents.md'),
        '# Version 2',
      );
    ...
    it('should only sync .md files, ignoring other file types', () => {
      ...
      expect(existsSync(join(tempDir, '.claude/guidance/notes.txt'))).toBe(false);
    });
    ...
    it('should create .claude/guidance/ directory if missing', () => {
      ...
      const result = simulateAutoUpdate(tempDir);
      expect(result.guidanceSynced).toBe(1);
      expect(existsSync(join(tempDir, '.claude/guidance/test.md'))).toBe(true);
    });
  });
