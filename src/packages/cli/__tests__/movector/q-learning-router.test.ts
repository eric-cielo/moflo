...
import { QLearningRouter, createQLearningRouter, type RouteDecision } from '../../src/movector/q-learning-router';
...
describe('QLearningRouter', () => {
  ...
  describe('initialize', () => {
    it('should initialize without native backend (fallback mode)', async () => {
      await router.initialize();
      const stats = router.getStats();
      expect(stats.useNative).toBe(0);
    });

    it('should handle native backend unavailability gracefully', async () => {
      await router.initialize();
      const stats = router.getStats();
      expect(stats.useNative).toBe(0);
    });
