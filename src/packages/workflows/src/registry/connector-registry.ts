 ...
 * 2. Shipped: `src/packages/workflows/connectors/` (bundled with moflo)
...
function isValidConnector(obj: unknown): obj is WorkflowConnector {
...
export class WorkflowConnectorRegistry {
  ...
  register(connector: WorkflowConnector, source: ConnectorSource = 'shipped'): void {
    if (!isValidConnector(connector)) {
      throw new Error('Object does not implement WorkflowConnector interface');
    }
  ...
  get(name: string): WorkflowConnector | undefined {
  ...
  async scan(): Promise<ConnectorScanResult> {
    ...
    const byName = new Map<string, { connector: WorkflowConnector; source: ConnectorSource }>();
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (!isValidConnector(connector)) {
          errors.push({ file: candidate.file, message: 'Does not implement WorkflowConnector interface' });
          continue;
        }
