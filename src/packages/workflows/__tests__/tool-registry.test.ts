...
function makeConnector(name: string, overrides?: Partial<WorkflowConnector>): WorkflowConnector {
  return {
    name,
...
describe('WorkflowConnectorRegistry', () => {
  ...
  it('throws on invalid connector (missing required methods)', () => {
    const invalid = { name: 'bad', description: 'x' } as unknown as WorkflowConnector;
    expect(() => registry.register(invalid)).toThrow(
      'does not implement WorkflowConnector interface'
    );
  });
  ...
  it('scan with nonexistent directory returns zero registered', async () => {
    const reg = new WorkflowConnectorRegistry({
...
describe('WorkflowConnectorRegistry npm discovery', () => {
  let tmpDir: string;
  ...
  it('discovers moflo-connector-* packages from node_modules', async () => {
    ...
    const reg = new WorkflowConnectorRegistry({ projectRoot: tmpDir });
    const result = await reg.scan();
  ...
  it('no node_modules directory does not crash', async () => {
    const reg = new WorkflowConnectorRegistry({ projectRoot: path.join(tmpDir, 'empty-project') });
    const result = await reg.scan();
