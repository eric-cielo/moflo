...
```ts
import { createCapabilityAlgebra } from '@moflo/guidance/capabilities';
...
```ts
import { getTrustBasedRateLimit } from '@moflo/guidance/trust';
...
```ts
import { createAuthorityGate, createIrreversibilityClassifier } from '@moflo/guidance/authority';
