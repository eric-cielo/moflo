 ...
 *
 * Provides code dependency graph analysis using movector's graph algorithms:
 ...
 *
 * @module moflo/movector/graph-analyzer
 */
...
// ============================================================================
// MoVector Integration (with graceful fallback)
// ============================================================================

/**
 * Interface for movector graph operations
 */
interface IMoVectorGraph {
...
let moVectorGraph: IMoVectorGraph | null = null;
let moVectorLoadAttempted = false;

/**
 * Load graph algorithms (pure TS fallbacks are always used)
 */
async function loadMovector(): Promise<IMoVectorGraph | null> {
  if (moVectorLoadAttempted) return moVectorGraph;
  moVectorLoadAttempted = true;
  // All graph analysis uses built-in pure TS implementations
  return null;
}
...
/**
 * Stoer-Wagner MinCut algorithm (fallback when native backend not available)
...
/**
 * Louvain community detection algorithm (fallback when native backend not available)
...
export async function analyzeMinCutBoundaries(
  ...
  // Try native backend, fallback to built-in
  const moVector = await loadMovector();

  // Get initial partition
  let result: ReturnType<typeof fallbackMinCut>;
  if (moVector) {
    result = moVector.mincut(nodes, edges);
  } else {
  ...
  if (numPartitions > 2 && result.partition1.length > 2) {
    ...
    const subResult = moVector
      ? moVector.mincut(result.partition1, subEdges)
...
export async function analyzeModuleCommunities(graph: DependencyGraph): Promise<ModuleCommunity[]> {
  ...
  // Try native backend, fallback to built-in
  const moVector = await loadMovector();
  const result = moVector ? moVector.louvain(nodes, edges) : fallbackLouvain(nodes, edges);
