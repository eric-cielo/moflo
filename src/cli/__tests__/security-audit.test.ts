...
describe('No Hardcoded Secrets', () => {
  ...
  // Test fixtures (under __tests__/) intentionally include fake API-key
  // formats to exercise the redaction logic — skip them.
  const SCAN_SKIP_DIRS = new Set(['node_modules', '__tests__', '__mocks__', 'tests', 'dist']);
...
describe('Plugin Manager NPM Injection', () => {
  it('should use execFileAsync and validate package names for plugin install (fixed S-3)', () => {
    ...
    const managerPath = path.join(__dirname, '..', 'plugins', 'manager.ts');
